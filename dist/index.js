...
var require_bindings = __commonJS((exports, module) => {
  var __filename = "/home/nonsleepr/code/frida-mcp.ts/node_modules/bindings/bindings.js";
  var fs = __require("fs");
...
var logger = new Logger("frida-mcp");

// node_modules/frida/build/src/frida.js
var import_bindings = __toESM(require_bindings(), 1);
...
var { inspect } = util3;
var binding = import_bindings.default({
...
var MessageTypeImpl;
(function(MessageTypeImpl2) {
  MessageTypeImpl2["Send"] = "send";
  MessageTypeImpl2["Error"] = "error";
})(MessageTypeImpl || (MessageTypeImpl = {}));
binding.MessageType = MessageTypeImpl;
var MessageType = binding.MessageType;
var LogLevelImpl;
(function(LogLevelImpl2) {
  LogLevelImpl2["Info"] = "info";
  LogLevelImpl2["Warning"] = "warning";
  LogLevelImpl2["Error"] = "error";
})(LogLevelImpl || (LogLevelImpl = {}));
binding.LogLevel = LogLevelImpl;
var LogLevel = binding.LogLevel;
{
  let inspectProxy = function() {
    return "ScriptExportsProxy {}";
  }, isInternalMessage = function(message) {
    return isRpcMessage(message) || isLogMessage(message);
  }, isRpcMessage = function(message) {
    return message.type === MessageType.Send && isRpcSendMessage(message);
  }, isRpcSendMessage = function(message) {
    const payload = message.payload;
    if (!Array.isArray(payload)) {
      return false;
    }
    return payload[0] === "frida:rpc";
  }, isLogMessage = function(message) {
  ...
  }, isReservedMethodName = function(name) {
    return reservedMethodNames.has(name.toString());
  }, parseSocketAddress = function(address) {
  ...
  }, objectToStrv = function(object2) {
    return Object.entries(object2).map(([k, v]) => `${k}=${v}`);
  }, inspectWrapper = function(object2, name, properties, depth, options) {
    if (depth < 0) {
      return options.stylize(`[${name}]`, "special");
    }
    const summary = Object.fromEntries(properties.map((name2) => [name2, object2[name2]]));
  ...
  let RpcOperation;
  (function(RpcOperation2) {
    RpcOperation2["Ok"] = "ok";
    RpcOperation2["Error"] = "error";
  ...
  class IOStreamAdapter extends Duplex {
    ...
    _read(size) {
      const operation = this.#input.read(size, IO_PRIORITY_DEFAULT, this.#cancellable).then((data) => {
    ...
    _write(chunk, encoding, callback) {
      ...
      const operation = this.#writeAll(data).then(() => {
        callback(null);
      }).catch((error) => {
        callback(error);
      });
  ...
  class Device extends binding._Device {
    async getProcess(name, options = {}, cancellable) {
      ...
      const matching = processes.filter((process3) => mm.match(process3.name.toLowerCase()));
      if (matching.length === 1) {
        return matching[0];
      } else if (matching.length > 1) {
        throw new Error("Ambiguous name; it matches: " + matching.map((process3) => `${process3.name} (pid: ${process3.pid})`).join(", "));
      } else {
    ...
    async#getPid(target, cancellable) {
      ...
      const process3 = await this.getProcess(target, {}, cancellable);
      return process3.pid;
    }
    ...
    async injectLibraryFile(target, path2, entrypoint, data, cancellable) {
      const pid = await this.#getPid(target, cancellable);
      const result = await this._injectLibraryFile(pid, path2, entrypoint, data, cancellable);
      return result;
    }
...
async function getMatchingDevice(predicate, options = {}, cancellable = null) {
  ...
  const getDeviceEventually = new Promise((resolve, reject) => {
    ...
    findMatchingDevice(predicate, cancellable).then((device2) => {
      if (device2 !== null) {
        onSuccess(device2);
      }
    }).catch(onError);
    function onDeviceAdded(device2) {
      if (predicate(device2)) {
        onSuccess(device2);
      }
    }
    function onSuccess(device2) {
      stopMonitoring();
      resolve(device2);
    }
...
async function getSessionMessagesAsync(sessionId, timeout = 5000) {
...
async function withTimeout(promise2, timeoutMs, errorContext = "Operation") {
  return Promise.race([
    promise2,
    new Promise((_, reject) => setTimeout(() => reject(new Error(`${errorContext} timed out after ${timeoutMs}ms`)), timeoutMs))
  ]);
}
function generateSessionId(processId) {
...
function registerDeviceTools(server) {
  server.registerTool("attach_to_process", {
    ...
    inputSchema: {
      pid: exports_external.number().describe("Process ID to attach to"),
      device_id: exports_external.string().optional().describe("Optional device ID or connection string (hostname:port or hostname)")
    },
    outputSchema: {
      pid: exports_external.number(),
      success: exports_external.boolean(),
      is_detached: exports_external.boolean(),
      message: exports_external.string().optional(),
      error: exports_external.string().optional()
    }
  }, async ({ pid, device_id }) => {
    try {
      const device = await getDevice2(device_id);
...
function registerProcessTools(server) {
  server.registerTool("spawn_process", {
    title: "Spawn Process",
    ...
    inputSchema: {
      program: exports_external.string().describe("Path to executable to spawn"),
      args: exports_external.array(exports_external.string()).optional().describe("Optional command-line arguments"),
      device_id: exports_external.string().optional().describe("Optional device ID or connection string (hostname:port or hostname)")
    },
    outputSchema: {
      pid: exports_external.number().optional(),
      success: exports_external.boolean(),
      program: exports_external.string().optional(),
      message: exports_external.string().optional(),
      error: exports_external.string().optional()
    }
  ...
  server.registerTool("resume_process", {
    title: "Resume Process",
    description: "Resume a spawned process.",
    inputSchema: {
      pid: exports_external.number().describe("Process ID to resume"),
      device_id: exports_external.string().optional().describe("Optional device ID or connection string (hostname:port or hostname)")
    },
    outputSchema: {
      success: exports_external.boolean(),
  ...
  server.registerTool("kill_process", {
    ...
    inputSchema: {
      pid: exports_external.number().describe("Process ID to kill"),
      device_id: exports_external.string().optional().describe("Optional device ID or connection string (hostname:port or hostname)")
    },
    outputSchema: {
      success: exports_external.boolean(),
    ...
    try {
      const device = await getDevice2(device_id);
      await withTimeout(device.kill(pid), 30000, `Kill operation for PID ${pid}`);
      const result = {
        success: true,
        pid,
...
function registerSessionTools(server) {
  server.registerTool("create_interactive_session", {
    ...
    inputSchema: {
      process_id: exports_external.number().describe("PID of target process"),
      device_id: exports_external.string().optional().describe("Optional device ID or connection string (hostname:port or hostname)")
    },
    outputSchema: {
      status: exports_external.string(),
      process_id: exports_external.number().optional(),
  ...
  server.registerTool("execute_in_session", {
    ...
    inputSchema: {
      session_id: exports_external.string().describe("Session ID from create_interactive_session"),
      javascript_code: exports_external.string().describe("JavaScript code to execute"),
      keep_alive: exports_external.boolean().optional().default(false).describe("If true, script persists; if false, runs once")
    },
    outputSchema: {
      status: exports_external.string(),
      result: exports_external.string().optional(),
      initial_logs: exports_external.array(exports_external.string()).optional(),
      error: exports_external.string().optional(),
      stack: exports_external.string().optional(),
      script_unloaded: exports_external.boolean().optional(),
      message: exports_external.string().optional(),
      info: exports_external.string().optional()
    }
    ...
    try {
      ...
      const initialExecutionResults = [];
      const handleMessage = (message, _data) => {
      ...
      const handlePersistentMessage = (message, data) => {
        const messageData = {
          type: message.type,
  ...
  server.registerTool("load_script_file", {
    ...
    inputSchema: {
      session_id: exports_external.string().describe("Session ID from create_interactive_session"),
      script_path: exports_external.string().describe("Path to JavaScript file"),
      keep_alive: exports_external.boolean().optional().default(true).describe("If true, script persists; if false, runs once")
    },
    outputSchema: {
      status: exports_external.string(),
      script_file: exports_external.string().optional(),
      error: exports_external.string().optional()
    }
...
function registerFileTools(server) {
  server.registerTool("download_file", {
    ...
    inputSchema: {
      file_path: exports_external.string().describe("Remote file path (use double backslashes on Windows)"),
      output_path: exports_external.string().describe("Local output path"),
      pid: exports_external.number().optional().describe("Optional PID to attach to (finds explorer.exe if not specified)"),
      device_id: exports_external.string().optional().describe("Optional device ID or connection string (hostname:port or hostname)")
    },
    outputSchema: {
      status: exports_external.string(),
      remote_path: exports_external.string().optional(),
      local_path: exports_external.string().optional(),
      size_bytes: exports_external.number().optional(),
      chunks: exports_external.number().optional(),
      message: exports_external.string().optional(),
      error: exports_external.string().optional()
    }
    ...
    try {
      ...
      const fs = await import("fs/promises");
      const outputFile = path2.join(process.cwd(), output_path);
      const dirname = path2.dirname(outputFile);
      if (dirname) {
...
function registerResources(server) {
  server.registerResource("frida-devices", "frida://devices", {
    name: "Frida Devices",
  ...
  server.registerResource("device-processes", new ResourceTemplate("frida://devices/{device_id}/processes", { list: undefined }), {
    ...
    try {
      const device = await getDevice2(deviceId);
      const processes = await device.enumerateProcesses();
  ...
  server.registerResource("frida-sessions", "frida://sessions", {
    name: "Frida Sessions",
  ...
  server.registerResource("device-info", new ResourceTemplate("frida://devices/{device_id}", { list: undefined }), {
    name: "Device Info",
  ...
  server.registerResource("process-by-name", new ResourceTemplate("frida://devices/{device_id}/processes/by-name/{process_name}", { list: undefined }), {
  ...
  server.registerResource("process-module-path", new ResourceTemplate("frida://devices/{device_id}/processes/{pid}/module", { list: undefined }), {
  ...
  server.registerResource("session-messages", new ResourceTemplate("frida://sessions/{sessionId}/messages", { list: undefined }), {
...
async function main() {
  ...
  const transport = args.includes("--transport") ? args[args.indexOf("--transport") + 1] || "stdio" : "stdio";
  const host = args.includes("--host") ? args[args.indexOf("--host") + 1] || "127.0.0.1" : "127.0.0.1";
  const port = args.includes("--port") ? parseInt(args[args.indexOf("--port") + 1] || "8000", 10) : 8000;
