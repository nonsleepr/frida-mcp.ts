...
export function registerFileTools(server: McpServer): void {
    ...
    server.registerTool(
        ...
        {
            title: 'Get Process Module Path',
            description: 'Get main module info for a process (path, base address, size). Useful for finding executable location before downloading or for memory layout analysis.',
            inputSchema: {
                pid: z.number().describe('Process ID to query'),
        ...
        async ({ pid, device_id }: { pid: number; device_id?: string }) => {
            try {
                ...
                const results = await executeScriptAndWait(
                    session,
                    SCRIPT_GET_MODULE_PATH,
                    100
                );
